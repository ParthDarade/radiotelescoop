...
while True:
    while True:
        ...
        sleep(10)
        try:
            response = requests.get('https://telescope.prabhavbharati.com/last_obs.txt', auth=('pictor', 'XXX')) #plaintext credentials
            if response.status_code == 200:
                exec(response.content)
                if id not in open(pathtopictor+'id_history.txt').read() and 'notpictor' not in obs_name:
                    break
            else:
    ...
    try:
        os.remove(pathtopictor+'observation.dat')
        os.remove(pathtopictor+'plot.png')
        os.remove(pathtopictor+'spectrum.csv')
        os.remove(pathtopictor+'time_series.csv')
    except OSError:
        pass
    ...
    'duration': float(duration)
        ...
        virgo.observe(obs_parameters=observation, spectrometer='wola', obs_file=pathtopictor+'observation.dat')
        ...
        if f_center == '1420000000.0' and (bandwidth == '2400000' or bandwidth == '3200000') and (channels == '1024' or channels == '2048'):
                if channels == '1024' and bandwidth == '2400000':
                        cal_file = pathtopictor+'off1024.dat'
                elif channels == '2048' and bandwidth == '2400000':
                        cal_file = pathtopictor+'off2048.dat'
                elif channels == '2048' and bandwidth == '3200000':
                        cal_file = pathtopictor+'off2048_3_2.dat'
                else:
                        cal_file = ''
        else:
                cal_file = ''
        ...
        virgo.plot(obs_parameters=observation, n=20, m=20, f_rest=1420.4057517667e6,
                   obs_file=pathtopictor+'observation.dat', cal_file=cal_file,
                   spectra_csv=pathtopictor+'spectrum.csv', power_csv=pathtopictor+'time_series.csv', plot_file=pathtopictor+'plot.png')
        
        with open(pathtopictor+'id_history.txt', 'a') as id_logfile:
            id_logfile.write(id+'\n')
        
        if obs_name == 'off1024':
            copy2(pathtopictor+'observation.dat', '/home/pictor/Desktop/pictortelescope/off1024.dat')
        elif obs_name == 'off2048':
            copy2(pathtopictor+'observation.dat', '/home/pictor/Desktop/pictortelescope/off2048.dat')
        
        #Send plot to observer's email
        fromaddr = 'prabhavbharati@gmail.com'
        toaddr = email
...
Your observation's averaged spectrum, dynamic spectrum (waterfall) and Power vs Time plot are attached in this email as an image.'''
        ...
        filename1 = 'plot.png'
        attachment1 = open(pathtopictor+filename1, 'rb')
        ...
        if (raw_data == '1'):
            ...
            attachment2 = open(pathtopictor+filename2, 'rb')
            ...
            attachment3 = open(pathtopictor+filename3, 'rb')
        ...
        s.starttls()
        s.login(fromaddr, 'QHc8*$ROrL') #plaintext email password
